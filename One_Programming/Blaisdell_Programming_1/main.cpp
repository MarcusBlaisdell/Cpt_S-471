                ...
                int *matchCount, int *mismatchCount, int *gapCount, int *openingGap)
{
  ...
  for (i = 1; i < n; i++)
  {
    for (j = 1; j < m; j++)
    {
      ...
      del = ((myTable[i-1][j-1].del_score) + g);
      ...
      ins = ((myTable[i-1][j-1].ins_score) + g);
      ...
      myTable[i][j].sub_score = max + ( S(S1[i-1], S2[j-1]) );
...
void runLocal (string S1, string S2, int match, int mismatch, int g, int h,
                int *matchCount, int *mismatchCount, int *gapCount, int *openingGap)
{
  // We need a matrix:
...
int main (int argc, char * argv [])
{
  ...
  else if (strcmp (argv[2], "1") == 0)
  {
    ...
    runLocal (S1, S2, match, mismatch, g, h,
  ...
  cout << "(" << ( (100) * (matchCount / S1.length() ) ) << "%), Gaps = ";
  cout << gapCount << "/" << S1.length() << "(";
  cout << ( (100) * (gapCount / S1.length() ) ) << "%)" << endl << endl;
