  ...
  if (theString[i])
  {
    ...
    else
    {
      ...
      else
      {
        ...
        alpha = curPtr->child[childNum]->edgeLabel[1];
        ...
        if (alpha < len)
        {
          ...
          for (j = 0; j < alpha; j++)
          {
            ...
            cout << "  -  theString[alpha + j]]: ";
            cout << theString[alpha + j] << endl;
            #endif

            if (theString[i] == theString[alpha + j])
            {
          ...
          pathFound = findPath (curPtr, i + alpha, len - alpha);
...
void insertSuffix (int i, int len)
{
  #ifdef DEBUG
  cout << "\n\t*** insertSuffix called ***\n" << endl;
  cout << "curPtr->nodeID: " << curPtr->nodeID;
      ...
      else
      {
        ...
        if (alpha < len)
        {
          for (j = 0; j < alpha; j++)
          {
            ...
            if (theString[i + j] != theString[alpha + j])
            {
              #ifdef DEBUG1
              cout << "\t************* mismatch *****************" << endl;
              cout << "theString[i]: " << theString[i];
          ...
          curPtr = curPtr->child[childNum];
          //pathFound = findPath (curPtr->child[childNum], i + alpha[1], len - alpha[1]);
          insertSuffix (i + alpha, len - alpha);
...
void add_v_edge (int i, int len, int childNum, int theBeta)
{
  #ifdef DEBUG1
  cout << "\n\t*** add_v_edge called ***\n" << endl;
  cout << "curPtr->nodeID: " << curPtr->nodeID;
  ...
  newInternalNode->parent = curPtr;
    ...
    newInternalNode->edgeLabel[0] = curPtr->child[childNum]->edgeLabel[0];
    newInternalNode->edgeLabel[1] = theBeta;
    newInternalNode->stringDepth = curPtr->stringDepth + theBeta;
    if (newInternalNode->stringDepth == 1)
    {
      newInternalNode->SL = curPtr->parent;
    } // end if stringDepth == 1

    /*
    newLeafNode->nodeID = leafID++;
    newLeafNode->parent = newInternalNode;
    newLeafNode->edgeLabel[0] = i + theBeta;
    newLeafNode->edgeLabel[1] = len - theBeta;
    newLeafNode->stringDepth = curPtr->stringDepth + len;
    */

      // update the child edgeLabel information:

    curPtr->child[childNum]->edgeLabel[0] += theBeta;
    curPtr->child[childNum]->edgeLabel[1] -= theBeta;
  ...
  // add_v_child function

void add_v_child (int i, int len, int theBeta)
{
  #ifdef DEBUG1
  cout << "\n\t*** add_v_child called ***\n" << endl;
...
void NodeHops (int i, int len, int theBeta)
{
  #ifdef DEBUG1
  cout << "\n\t*** NodeHops called ***\n" << endl;
  printPointers ();
  ...
  else
  {
    ...
    for (k = 0; k < j; k++)
    {
      ...
      if (theString[i] == theString[curPtr->child[k]->edgeLabel[0]])
      {
        ...
        #ifdef DEBUG1
        cout << "NodeHops found a child at: " << childNum << endl;
        #endif
    ...
    if (found == 1)
    {
      ...
      } // end if edge length greater than beta

        // if the edge length is less than beta
        // we can hop to the next node and recursively
        // call NodeHops:

      else if (curPtr->child[childNum]->edgeLabel[1] < theBeta)
      {
          // if the edge length is less than theBeta,
        ...
        theBeta -= curPtr->child[childNum]->edgeLabel[1];
        ...
        curPtr = curPtr->child[childNum];
          ...
          // recursively call NodeHops until all node hops are complete
        #ifdef DEBUG1
        cout << "recursive call to NodeHops:" << endl;
        cout << "curPtr->nodeID: " << curPtr->nodeID;
        cout << " : curPtr->parent->nodeID: " << curPtr->parent->nodeID;
        cout << endl;
        #endif

        NodeHops (i, len, theBeta);
      ...
      // else, edge length matches theBeta:
      // if the edge length matches theBeta, next node is v,
      ...
      } // end else, edgeLabel length matches theBeta

    } // end if matching child

    // if curPtr does not have a child
    // that starts with the first character
    // of our string,
    // add as child of curPtr:
...
void case_IIA (int i, int len)
{
  ...
  cout << "curPtr->nodeID: " << curPtr->nodeID;
  cout << " : i: " << i << " - len: " << len << endl;
  cout << "u->edgeLabel[1]: " << u->edgeLabel[1] << endl;
  #endif
  ...
  cout << "u_prime->parent->nodeID: " << u_prime->parent->nodeID << endl;
    ...
    // alpha_prime is alpha - c
    // where c is the first character of alpha
    // which is the edge label of u'
    // I just need it's length, not its
    // start character:

  //alpha_prime[0] = u->edgeLabel[0] + (u_prime->stringDepth - 1);
  alpha_prime = u_prime->stringDepth - 1;
  ...
  {
    cout << "SL is null" << endl;
    //v_prime = u_prime->parent->SL;
    //alpha_prime = u_prime->parent->stringDepth;
  }
  else
  {
    cout << "SL is fine" << endl;
  }
  ...
  //alpha_prime = len -= v_prime->stringDepth;
  ...
  #ifdef DEBUG1
  cout << "case_IIA calling NodeHops" << endl;
  #endif
...
void case_IIB (int i, int len)
{
  ...
  #ifdef DEBUG1
  cout << "case_IIB calling NodeHops" << endl;
  #endif

  NodeHops (i, len, beta_prime);
...
void getExactHelper (aNode *, int);

  // getExact function
  // finds the longest exact matching repeat
  // and reports its length and coordinates
  // the longest exact matching repeat is found
  // as the string depth of the lowest internal node
  // that is, the internal node with the greatest
  // string depth
  // by subtracting the depth size from the start index
  // of all of its children, we can get the coordinates
  // of all of the matches in the string

void getExact (aNode * u)
{
  #ifdef DEBUG
  cout << "getExact called " << endl;
  #endif
  ...
  int e = exactRepeat->stringDepth;

  cout << "\n\n\t\t*** Longest exact matching repeat ***\n\n";
  cout << "The longest exact matching repeat is : ";
  cout << e;
  cout << " : characters long\n" << endl;
  cout << "The coordinates are: ";

  j = exactRepeat->child.size ();

  for (k = 0; k < j; k++)
  {
    cout << exactRepeat->child[k]->edgeLabel[0] - e;
    cout << ", ";
  } // end for loop to get all coordinates

  cout << "\n\n" << endl;

} // end getExact function

  // getExactHelper function

void getExactHelper (aNode * u, int ifFirst)
{
  #ifdef DEBUG
  cout << "getExactHelper called" << endl;
  #endif
  int j = 0, k = 0;
  ...
  if (j > 0)
  {
    for (k = 0; k < j; k++)
    {
      if (u->child[k]->nodeID <= theString.size ())
      {
        // ignore leaf nodes
      } // end it is a leaf
      else
      {
        exactDepth = u->child[k]->stringDepth;
        exactSum += exactDepth;

        if (exactDepth > exactDeepest)
        {
          exactDeepest = exactDepth;

          #ifdef DEBUG
          cout << "deepest: " << exactDeepest << endl;
          cout << "u->child[k]->nodeID: " << u->child[k]->nodeID << endl;
          #endif

          exactRepeat = u->child[k];

        } // end update deepest

      } // end else it is internal

      if (u->child[k]->child.size () > 0)
      {
        getExactHelper (u->child[k], 1);

      } // end if node has children, recursively evaluate them

    } // end print out all children

  } // end else, evaluate all children
