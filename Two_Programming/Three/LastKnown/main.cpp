...
bool findPath (aNode * theNode, int i, int len)
{
  ...
  if (theString[i])
  {
    ...
    else
    {
      ...
      {
        #ifdef DEBUG
        cout << "findPath: found == 0, curPtr->nodeID: " << curPtr->nodeID << endl;
        #endif
      ...
      else
      {
        ...
        {
          // if the label is longer than evaluation string,
...
void insertNode (int i, int len)
{
  ...
  else
  {
    ...
    if (found == 1)
    {
      cout << "insertNode, child-path found, " << numChild << endl;
      //i += curPtr->stringDepth;
      //len -= curPtr->stringDepth;

      insertEdge (i, len, numChild);
...
void insertChild (int i, int len)
{
  ...
  // insertEdge function
void insertEdge (int i, int len, int numChild)
{
  #ifdef DEBUG1
  cout << "\n\t*** insertEdge called ***\n" << endl;
  ...
  //i += curPtr->edgeLabel[1];
  //len -= curPtr->edgeLabel[1];

  int j = 0;

  while (theString[i + j] == theString[curPtr->child[numChild]->edgeLabel[0] + j])
  {
    #ifdef DEBUG1
    cout << "i: " << i;
    cout << " : curPtr->child[numChild]->edgeLabel[0] ";
    cout << curPtr->child[numChild]->edgeLabel[0];
    cout << " + j: " << j << endl;
    cout << "theString[i]: " << theString[i];
    cout << " : theString[curPtr->child[numChild]->edgeLabel[0] + j]: ";
    cout << theString[curPtr->child[numChild]->edgeLabel[0] + j];
    cout << endl;
    #endif
    j++;

  } // end move to insert position
  ...
  newInternalNode->edgeLabel[0] = curPtr->child[numChild]->edgeLabel[0];
  newInternalNode->edgeLabel[1] = j;
  ...
  newLeafNode->edgeLabel[0] = i + j;
  newLeafNode->edgeLabel[1] = len - j;
  newLeafNode->stringDepth = curPtr->stringDepth + len;
  cout << "i: " << i << " : j: " << j << " : i + j: " << i + j << endl;

  curPtr->child[numChild]->edgeLabel[0] += j;
  curPtr->child[numChild]->edgeLabel[1] -= j;
  curPtr->child[numChild]->parent = newInternalNode;


    // insert-in-order newLeafNode and
    // curPtr->child[numChild] to
    // newInternalNode->child list:

  #ifdef DEBUG1
  cout << "theString[newLeafNode->edgeLabel[0]]: ";
  cout << theString[newLeafNode->edgeLabel[0]];
  cout << " : theString[curPtr->child[numChild]->edgeLabel[0]]: ";
  cout << theString[curPtr->child[numChild]->edgeLabel[0]] << endl;
  #endif

  if ((theString[newLeafNode->edgeLabel[0]] == '$') ||
      (theString[newLeafNode->edgeLabel[0]] <
      theString[curPtr->child[numChild]->edgeLabel[0]]) )
  {
    newInternalNode->child.push_back (newLeafNode);
    newInternalNode->child.push_back (curPtr->child[numChild]);
  ...
  } // end else, insert curPtr->child[numChild] first

    // update curPtr->child[numChild] to point
    // to the new internal node:

  curPtr->child[numChild] = newInternalNode;
...
void add_v_edge (int i, int len, int numChild)
{
  #ifdef DEBUG1
  cout << "\n\t*** add_v_edge called ***\n" << endl;
  cout << "curPtr->nodeID: " << curPtr->nodeID;
  ...
  */
  ...
  //{
    ...
    while (theString[i + j] == theString[curPtr->child[numChild]->edgeLabel[0] + j] )
    {
      ...
      cout << " : theString[curPtr->child[numChild]->edgeLabel[0] + j]: ";
      cout << theString[curPtr->child[numChild]->edgeLabel[0] + j] << endl;
      #endif
  ...
  else
  {
    ...
    for (k = 0; k < j; k++)
    {
      ...
      if (theString[i] == theString[curPtr->child[k]->edgeLabel[0]])
      {
        found = 1;
        numChild = k;
        cout << "NodeHopsA found a child at: " << numChild << endl;
        break;
    ...
    if (found == 1)
    {
      cout << "found child: " << numChild << endl;
      // evaluate child:
      // if it's edge length
      ...
      else if (curPtr->child[numChild]->edgeLabel[1] > len)
      {
      ...
      else if (curPtr->child[numChild]->edgeLabel[1] < len)
      {
        ...
        i += curPtr->child[numChild]->edgeLabel[1];
        len -= curPtr->child[numChild]->edgeLabel[1];

        beta[0] += curPtr->child[numChild]->edgeLabel[1];
        beta[1] -= curPtr->child[numChild]->edgeLabel[1];

          // perform the hop:
        cout << "curPtr->nodeID: " << curPtr->nodeID << endl;
        cout << "numChild: " << numChild << endl;
        cout << "curPtr->child[numChild]->nodeID: ";
        cout << curPtr->child[numChild]->nodeID << endl;
        curPtr = curPtr->child[numChild];
...
else
{
  ...
  for (k = 0; k < j; k++)
  {
    ...
    if (theString[i] == theString[curPtr->child[k]->edgeLabel[0]])
    {
      found = 1;
      numChild = k;
      cout << "NodeHopsB found a child at: " << numChild << endl;
      break;
  ...
  if (found == 1)
  {
    ...
    if (curPtr->child[numChild]->edgeLabel[1] > len)
    {
        // v doesn't exist, add v:
      #ifdef DEBUG1
      cout << "v does not exist, calling add_v_edge" << endl;
...
void case_Naive (int i, int len)
{
  ...
  pathFound = findPath (curPtr, i, len);
