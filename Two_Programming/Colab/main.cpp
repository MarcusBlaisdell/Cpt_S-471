...
{
  #ifdef DEBUG2
  cout << " - - Just opened findPath - - : " << endl;
  #endif

  #ifdef DEBUG
  cout << "~ ~ ~ findPath: parent->nodeID: " << parent->nodeID << endl;
  //cout << ", (" << parent->edgeLabel[0];
  //cout << ", " << parent->edgeLabel[1] << ")" << endl;
  ...
  if (stringLength == 0)
  {
    ...
    #ifdef DEBUG2
    cout << " - - leaving findPath - - : stringLength is 0" << endl;
    #endif
  ...
  if (theData[startIndex] == '$')
  {
    ...
    #ifdef DEBUG2
    cout << " - - leaving findPath - - : just $" << endl;
    #endif
  ...
  //vector <aNode *>::iterator it = parent->child.begin();
    ...
    if (found == 1)
    {
      ...
      for (j = 0; j < k; j++)
      {
            ...
            != theData[startIndex + j]) || ((stringLength - j) == 0))
        {
          ...
          #ifdef DEBUG2
          cout << " - - leaving findPath - - : mismatch found" << endl;
          #endif
    ...
    #ifdef DEBUG2
    cout << " - - leaving findPath - - : no path found" << endl;
    #endif
                ...
                int stringLength)
{
  ...
  #ifdef DEBUG2
  cout << " - - just entered NodeHops - - : ";
  cout << u->nodeID << ", (" << u->edgeLabel[0];
  ...
  {
    edgePos = 0;
    insertNode (u, startIndex, stringLength);
    return;
  ...
  for (j = 0; j < i; j++)
  {
    ...
    if (theData[u->child[j]->edgeLabel[0]] == theData[beta[0]])
    {
      ...
      if (beta[1] == u->child[j]->edgeLabel[1] )
      {
        u = u->child[j];
        #ifdef DEBUG
        cout << "\tequals Node Hop to " << u->nodeID << endl;
        #endif
        SLHolder->SL = u;
        startIndex += u->stringDepth;
        stringLength -= u->stringDepth;
        trunc = findPath (u, startIndex, stringLength, 0);
        insertNode (u, startIndex + trunc, stringLength - trunc);

        #ifdef DEBUG2
        cout << " - - leaving NodeHops - - : beta == edgeLength" << endl;
        #endif
      ...
      else if (beta[1] > u->child[j]->edgeLabel[1] )
      {
        while (beta[1] > u->child[j]->edgeLabel[1])
        {
          ...
          u = u->child[j];
          #ifdef DEBUG
          cout << "\tless than Node Hop to " << u->nodeID << endl;
          #endif
        ...
        if (beta[1] == u->child[j]->edgeLabel[1] )
        {
          ...
          #ifdef DEBUG2
          cout << " - - leaving NodeHops - - : less than, equal" << endl;
          #endif
        ...
        else
        {
          ...
          #ifdef DEBUG2
          cout << " - - leaving NodeHops - - : edgeLength > beta" << endl;
          #endif
...
cout << "here" << endl;
      ...
      else
      {
        ...
        #ifdef DEBUG
        cout << "\tv doesn't exist, adding at " << u->nodeID;
        cout << ", position " << edgePos << endl;
        #endif
        ...
        #ifdef DEBUG2
        cout << " - - leaving findPath - - : v didn't exist" << endl;
        #endif
  ...
  #ifdef DEBUG2
  cout << " - - leaving NodeHops - - : catch-all" << endl;
  #endif
...
void printTree (aNode *);

  // insertNode function

void insertNode (aNode * theNode, int startIndex, int stringLength)
{
  #ifdef DEBUG2
  cout << " - - Just opened insertNode - - ";
  cout << "u->nodeID: " << u->nodeID << ": (";
  cout << u->edgeLabel[0] << ", " << u->edgeLabel[1] << ")";
  cout << " : " << edgePos << endl;
  #endif
  ...
  if (edgePos > 0)
  {
    ...
    newInternalNode->edgeLabel[1] = edgePos;
    newInternalNode->stringDepth = theNode->parent->stringDepth + edgePos;
    ...
    theNode->edgeLabel[0] += edgePos;
    theNode->edgeLabel[1] -= edgePos;
    ...
    else
    {
      while (it != theNode->child.end() )
      {
        if (theData[theNode->child[i]->edgeLabel[0]] > theData[startIndex])
        {
          ...
          #ifdef DEBUG2
          cout << " - - leaving insertNode - - : inserted within child list" << endl;
          #endif
...
aNode * buildSFTree (string *theData)
{
  ...
  while (remLen > 0)
  {
    ...
    edgePos = 0;
    trunc = findPath (u, index, remLen, 0);
    insertNode (u, index + trunc, remLen - trunc);
    ...
    if (u->SL != NULL)
    {
      ...
      else
      {
        ...
        insertNode (u, newIndex + trunc, newRemLen - trunc);
...
int printDFS (aNode * u, int count, int ifFirst)
{
  ...
  if (u->edgeLabel[0] == -1 && ifFirst == 0)
  {
    ...
    #ifdef DEBUG1
    cout << u->nodeID << ", (" << u->edgeLabel[0] << ", ";
    cout << u->edgeLabel[1] << ") : " << u->stringDepth << endl;
    #else
  ...
  else
  {
    for (j = 0; j < i; j++)
    {
      ...
      #ifdef DEBUG1
      cout << u->child[j]->nodeID << ", (" << u->child[j]->edgeLabel[0] << ", ";
      cout << u->child[j]->edgeLabel[1] << ") : " << u->child[j]->stringDepth << endl;
      #else
...
void printTree (aNode * u)
{
  ...
  cout << "id=" << u->nodeID << "   parent=" << u->parent->nodeID << "   SL=";
