...
struct node //internal node + leaf
{
	...
	// p_edgelabel[0] = start index in string where the string begins
	// p_edgelabel[1] = length of string
	int p_edgelabel[2]; // **linear
...
node* NodeHops(int s[2], node* up); // **linear
...
node* FindPath(node* u, int s[2], int suf_i); // **linear
