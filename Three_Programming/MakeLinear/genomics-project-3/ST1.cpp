...
ST::ST(string *s, vector<char> *alphabet) // **linear
{
	input_string = s;
	//s.push_back('$'); //append '$' at the end of the string
	s.push_back('$'); //append '$' at the end of the string // **linear
	...
	int suffix_index = 0;
	//string cur_suffix = s;
	int s_index = 0; // **linear
	int s_length = s->length(); // **linear
	int cur_suffix[2]; // **linear
	cur_suffix[0] = s_index; // **linear
	cur_suffix[1] = s_length; // **linear

	node* u = NULL; //suffix leaf i-1 parent
	node* up = NULL; //parent of u

	//append the first suffix to the root (suffix_index = 0)
	node* new_leaf = create_leaf(cur_suffix, suffix_index); // **linear, this one doesn't change

	//new_leaf->p_edgelabel = s;
	new_leaf->p_edgelabel[0] = s_index; // **linear
	new_leaf->p_edgelabel[1] = s_length; // **linear
	...
	for (suffix_index = 1; suffix_index < s.length(); suffix_index++)
	{
		...
		{
			u = execute_IA(u, cur_suffix, suffix_index); // **linear, no change
		}
		else if (u->SL != NULL && u == &root) //case IB
		{
			u = execute_IB(u, cur_suffix, suffix_index); // **linear, no change
		}
		else if (u->SL == NULL && up != &root) //case IIA
		{
			u = execute_IIA(u, cur_suffix, suffix_index); // **linear, no change
		}
		else if (u->SL == NULL && up == &root) //case IIB
		{
			u = execute_IIB(u, cur_suffix, suffix_index); // **linear, no change
		}
		else
		{
...
void ST::prepareST(int lambda)
{
	...
	vector<int> A(input_string->length() + 1,-1); // **linear
	int nextIndex = 0;
	vector<int>::iterator it;
...
void build_BWT_index(node *u, vector<char>* BWT, string *s) // **linear
{
...
node* create_leaf(int cur_suffix[2], int suffix_index) // **linear
{
	node* new_leaf = new node;
	new_leaf->id = suffix_index;
	//new_leaf->depth = cur_suffix.length();
	new_leaf->depth = cur_suffix[1]; // **linear
	return new_leaf;
}
...
node* execute_IB(node* u, int cur_suffix[2], int index_suffix) // **linear
{
	node* v = u->SL;
...
node* execute_IIB(node* u, int cur_suffix[2], int index_suffix) // **linear
{
	node* up = u->parent;
	node* vp = up->SL;
...
node* execute_IIA(node* u, int cur_suffix[2], int index_suffix) // **linear
{
	node* up = u->parent;
	node* vp = up->SL;
	//string beta = u->p_edgelabel;
	int beta[2]; // **linear
	beta[0] = u->p_edgelabel[0]; // **linear
	beta[1] = u->p_edgelabel[1]; // **linear
	node* v = NodeHops(beta, vp);

	if (v == NULL)
	{
		//return FindPath(vp, cur_suffix.substr(vp->depth), index_suffix);
		cur_suffix[0] += vp->depth; // **linear
		cur_suffix[1] += vp->depth; // **linear
		return FindPath(vp, cur_suffix, index_suffix); // **linear
	}
	else
	{
		u->SL = v;
		//return FindPath(v, cur_suffix.substr(v->depth), index_suffix);
		cur_suffix[0] += v->depth; // **linear
		cur_suffix[0] -= v->depth; // **linear
		return FindPath(v, cur_suffix, index_suffix); // **linear
	}
}
...
node* execute_IA(node* u, int cur_suffix[2], int index_suffix) // **linear
{
	node* v = u->SL;
	//return FindPath(v, cur_suffix.substr(v->depth), index_suffix);
	cur_suffix[0] += v->depth; // **linear
	cur_suffix[1] -= v->depth; // **linear
	return FindPath(v, cur_suffix, index_suffix); // **linear
}
...
node* NodeHops(string *s, node* up) // **linear
{
	//if (s.length() == 0)
	if (s->length() == 0)
	{
	...
	int cur_edgelabel[2]; // **linear
	cur_edgelabel[0] = up->children[e]->p_edgelabel[0]; // **linear
	cur_edgelabel[1] = up->children[e]->p_edgelabel[1]; // **linear
	...
	if (s->length() < cur_edgelabel[1]) // **linear
	{
